...
\begin{enumerate}
    \item Download the \texttt{ontario\_road\_network.geojson} file shared via email- elias.alhazzaa@mail.utoronto.ca - and place it at the project's root directory.
...
\section{Results}
 The initial screen which opens up will load data and map, and then it would show the map at initial settings as shown below. This is the University of Toronto area along with a lot of other surrounding places,
\begin{center}
...
After the segment has been removed, press Compute Shortest Path method. The result would be that there is no shortest path, as removing the road leads to the two junctions getting disconnected.
\begin{center}
...
For the final submission, we wanted to create a more generalized version of the proposal. We had two major changes to our original proposal, in particular where our goal remained the same, building an app that allows an official to track how a road closure would affect traffic, but we did changes incorporating feedback to find better ways of achieving this goal.

The biggest change done was to use Dijktras to give all shortest paths, not just one shortest path. This required a study of the proof of Dijktras algorithm, come up with our own proof of Dijktras algorithm where it is shown that when a node is marked, it is
then by Dijkstra's algorithm, all of the shortest paths from the start point to the endpoint will be displayed on the map. Due to this change, we decided to use trees, along with graphs, to help us compute and store all shortest paths, and we added a dataclass called ShortestPathResults to handle the results of Dijkstra's algorithm. Therefore, trees, graphs, and dataclasses all play a significant role in our final submission for this project.

Our second major change from the proposal of our project was that instead of removing one road segment and finding any shortest path from the start point to the endpoint, we now give the user the choice to remove multiple segments. If the user chooses to compute shortest path, then in \texttt{Graph.check\_is\_valid\_road\_selection} method, we check with a custom small algorithm if sequence of roads is valid or not with a mini algorithm. We check validity by checking that there is at least one free start point (free meaning the start point is not the end point of another road) and at least one free end point (free meaning in this case that the end point is not the start point of another Road), and if there are more than one of roads with free start points and/or free end points, then we check if all such roads have the same start/end point respectively. This algorithm, if it decides the selection of roads as 'valid,' gives us a clean start junction id and end junction id between which we can compute the shortest distance. This algorithm allows the user to select multiple road segments, but in a manner that they are not arbitrarily floating everywhere hence making the idea of shortest path computation between two points meaningless. Since there was the possibility of by mistake selecting 'invalid' paths, we also created a reset button. Before this, Dijktras algorithm would have run after the road segment was actually removed from the graph, but now, it would have to be soft deleted to allow for a reset (which would include all deleted roads being restored). Hence, this change also forced us indirectly to add soft deletion, and then to modify Dijktras in a way that it ignores the soft deleted ones. This was not possible without actually understanding how the algorithm works, and hence this compounded the complexity of soft deletion.
