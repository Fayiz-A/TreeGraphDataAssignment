...
However, Graph alone does not entirely suffice. Because we are computing multiple shortest paths of same length (ignoring decimal part) from a source junction to a target junction, and the usage of trees helps us store and keep track of these paths. The class PathTree, found in path\_tree.py,  is a tree class to represent multiple paths from the source node to the target node. The target node is the root of the tree, and the leaves are the source nodes, so each path is traced from a leaf (a source node) all the way up to the root (the target node) of the tree. The reason it is upside down is because in our version of Dijktra's, it is a lot easier and efficient to track each shortest path back from target vertex to source then to do it the other way. PathTree was indispensable for this job, because it is not just about multiple shortest paths to target, but rather a lot of times there are also multiple equivalent shortest paths to another vertex which lies along the shortest path, thus giving the task of tracing all these paths a branching structure. Tree is a natural choice for this representation of multiple paths therefore. However, we don't return a tree back to our other classes, as it is cumbersome to traverse a tree to get a shortest path to display. Rather, after running Dijkstra’s algorithm to compute all the shortest paths from a source node to a target node, the returned object is an instance of the ShortestPathResult class in graph.py. This class is used to store all the possible paths from the source to the target node in its instance attribute, all\_shortest\_paths list. This list is populated by calling the function get\_all\_possible\_paths method of class PathTree, which returns all possible paths after the PathTree tree has been built by Dijkstra algorithm in the form of a list of list by storing each traversal from depth point of view from root to leaf in a separate list. All paths in this list have the shortest length from the source to the target node, which is stored in the instance attribute of the ShortestPathResult class, length. This is done in an optimized manner, as every root in PathTree and its subtrees has a tuple consisting of vertex id and road id that this vertex uses this connect to the parent (useful in case there are multiple direct roads of different length between two neighbouring vertices). A list is not used in our Dijktras algorithm to store a history of shortest paths directly, and rather this PathTree is used, as using a list all the way throughout Dijktras instead of storing just the immediate vertex which is part of shortest path in distance dict (see distance dict in Graph.py for more details) would have slowed it a lot. \\

Overall, Graph class is the backbone of our project, but it cannot do its job without PathTree.


Types of computations performed on the geojson data:

1. Displaying a graph representation of road network on Ontario map.

2. Making road segment(s) removable (soft delete) by clicking on it/them, using a modified version of Dijkstra's algorithm to compute all shortest paths of equal length (ignoring decimals for reasons discussed earlier) between point A and point B that the removed road segment(s) used to connect previously. Dijkstra's algorithm is used to give one shortest path from one
source vertex to all other ones, and it has a time complexity of $O(V+ELogV)$ where $E$ is the number of edges (roads) and $V$ is the number of vertices (intersections/road start/end point), so it is best suited to solve the problem efficiently for this data (GeeksforGeeks). Our modified version gives all shortest paths.

3. Using the same algorithm as above, building a collection of the shortest paths from the graph and highlighting these shortest paths on the map in a distinct color. If no path exists, the program displays a message indicating those two junctions/vertices are now disconnected.

4. Filtering and displaying only the part of graph which is currently in view, as drawing all polylines at once will lead to performance issues. Roads are filtered based on two criteria: whether any coordinate of the road falls within the current map bounds, and whether the road's length exceeds a threshold derived from the current zoom level. At zoom levels below 13, only roads longer than this threshold are shown. At zoom level 13 and above, all roads within bounds are shown. However, all roads present in currently displayed shortest path are displayed in the map regardless of any zoom or bounds\\

5. Restoring graph state to original one, so previous computations/deletions don't impact the future and fresh ones.

\textbf{Some notes on architecture of program:}

The \textbf{Streamlit} library's \texttt{st.session\_state} is used in \texttt{main.py} to persist the \texttt{StreamlitManager} instance across reruns. This way, main.py acts as a bootstrapping file only, with Streamlit related stuff mostly done in StreamlitManager class (Streamlit by default is skewed towards non object oriented code, so this needed bootstrapping this way). The StreamlitManager class' display method is what gets called when Streamlit runs the file from top to bottom, while the constructor only runs once, thus providing a convenient and familiar interface in this architecture designed by us to use Streamlit without it getting out of hand and making troubleshooting for bugs extremely tough.

StreamlitManager is our Boundary class, meaning it has absolutely no business logic and has its only job as displaying UI and handling logic related to the display of UI, and handling callbacks.

The RoadManager class is responsible for business logic, and it acts as a mediator between StreamlitManager and Graph class. The Graph class contains our algorithm code and low level logic related to handling road soft deletions, restorations, computing shortest path and so on.

\textbf{Libraries Used:}

The following Python libraries are used in this project to accomplish its tasks. The \textbf{Shapely} library is used for click detection on the map. Specifically, the \texttt{MultiLineString} class represents a road's geometric polyline as a Shapely geometry object, and the \texttt{distance} method of the \texttt{Point} class computes the shortest distance between a user's click point and each visible road's polyline in \texttt{StreamlitManager.\_get\_road\_id\_by\_selection} in \texttt{streamlit\_manager.py}. Since the \texttt{StreamlitManager.\_get\_road\_id\_by\_selection} is only called when some polyline is clicked, the job now is to figure out which line was clicked, which we do by computing distance to each polyline through Shapely and returning the road associated with the polyline with least distance to click. The \textbf{orjson} library's \texttt{orjson.loads} function in \texttt{fast\_geo\_json\_file\_loader.py} parses the big road network geojson file efficiently into a Python dictionary (a lot faster than json library does), which is then used by \texttt{RoadManager.fetch\_data\_and\_build\_graph} method in \texttt{road\_manager.py} to build the graph. 

The \textbf{Streamlit-Folium} library's \texttt{st\_folium} function renders the interactive map and captures user clicks, enabling bidirectional communication between the map and the app. Finally, the \textbf{Folium} library's \texttt{Map} class creates the base map of Ontario, and its \texttt{GeoJson} class draws each road segment using the \texttt{geometry} attribute of each \texttt{Road} object. \\

For UI, streamlit (on top of which streamlit\_folium is built) library's text method is used to show the length of new path, length of previous shortest path before road removal besides the map (Snowflake Inc., “Basic Concepts of Streamlit”; “St.text”).

An optimized and modified version of Dijkstra's algorithm is implemented in \texttt{Graph.compute\_shortest\_path} in \texttt{graph.py}. Unlike the standard algorithm which returns only one shortest path, this version collects all shortest paths of equal length using PathTree class (ignoring decimals for reasons discussed earlier). The result is returned as a \texttt{ShortestPathResult} dataclass containing the shortest length and all equal shortest paths. If no path exists between the two vertices, \texttt{None} is returned to indicate disconnection between source and target vertices. \\
...
Our last demo will be about what our app does when removing a road segment leads to junctions getting disconnected from each other. Click reset button to unselect our 2 selected road segments, and then scroll to Queen's Park area and click Reload Polylines if necessary. On the road id shown below in the tooltip (which comes up if one hovers over a road), click to remove it. 
\begin{center}
...
Clicking on button Reload Polylines: gets polylines as per your new bounds, map position and zoom level.

Selecting a road/polyline on map: it gets selected and marked for deletion if Compute Shortest Path button is pressed and the selection of roads overall is valid.

Clicking on Compute Shortest Path button: if the selection of roads is not valid, it sets state to InvalidRoadSelectionsState instance so as to alert the user about this. If it is valid, it computes shortest path by calling RoadManager.get\_shortest\_path method (which then calls Graph.compute\_shortest\_path method) to get the path before removal of road segments, and then calls RoadManager.remove\_road\_and\_get\_path to remove roads (soft deletion) and gets all shortest paths between start and end junctions decided by RoadManager.check\_removability method from sequence of selected roads.  It then sets state as 
JunctionDisconnectedState if removal of road led to the two junctions being disconnected, otherwise it sets state to instance of ShortestPathSuccessState with relevant details so display method of StreamlitManager class can render shortest path and show previous shortest path length without road removal, shortest path length after road removal.

Show Another Shortest Path Button: Only appears if there are more than one equivalent shortest paths, it calls StreamlitManager.\_shift\_to\_next\_shortest\_path method, which restores the roads currently in shortest path to normal colour and makes road in the next shortest path (next meaning the equivalent shortest path next in list, not next as in ascending) as per shortest path colouring. This helps user understand different equivalent shortest paths on the map that are equivalent without all of them being displayed together, which would have led to confusion.

Reset button: resets all roads to normal state (makes their colour as their default colour) and initialized other variables again so that the app can be used repeatedly in one session.

This demo covered all features this app has: computing and displaying shortest path after removal of one road segment, after removal of multiple road segments, computing multiple shortest paths of same length, reset feature, lazy loading of polylines for performance reasons, alerting if junctions get disconnected after road removal and flagging invalid sequence of roads. Now, the reader is free to experiment as the reader of this report likes with any of these features wherever the reader wants on the map.
...
The goal to create an app that allows a transportation person in charge to analyze how road closure affects traffic has been achieved to a degree. The following two example make it clear that a person in charge using this app can get an idea of effects of road closure on motorists in Ontario, just with a caveat discussed below. For road segments like the one near Queen's Park in our demo section, removing/closing which leads to the two junctions that the road segment used to connected getting disconnected, this program helps the person in charge immediately comprehend the vitality of that route, and hence that individual can then take steps to mitigate the closure to lessen its deep effects on the traffic where they cannot reach their destinations if they start at one of those junctions and have to go to the other junction. As a second example, consider the two equivalent shortest paths given on Spadina Dupont example. Both of them had length 504 meters, while before closure, the length was 309 meters. This is a trivial distance to cover for a motorist, but looking at both paths at the map, both of them have 2 turns, and hence despite less increase in difference, there would be considerable consequences for the motorists in this area due to swelling of intersections while cars turn, which leads to traffic slowdown. In more general terms, the combination of length of shortest map and displaying different shortest paths on map helps understand both the increase in distance and increase in number of turns for motorists, and hence gives a comprehensive picture for road closures.

However, despite this helping to achieve the goal to a degree, it does not give the complete picture. The complete picture would have been given by using Google Map's APIs to calculate average congestion and roads, and then modelling how the congestion now would increase over the roads above average congestion as a road segment is closed. This is because if the closure of shortest path leads users to use a road that was not used a lot previously anyways, this would mean that traffic flow would not have as much friction as anticipated by the map. 

Another way to improve is to use Google Map's APIs to use average time at specific time of day instead of road distance to determine which road users would take. This is from a motorist centric point of view. Motorists who use map applications don't take roads that are the shortest in distance, and rather take roads which are the shortest in time, and this is something in which the API would have helped in achieving. Thus, a Dijktras over average time a road segment takes from junction A to junction B would have given shortest path that the user is likely to take, and this means just some slight tweaks to this project can make it more comprehensive (assuming one is willing to pay the dollars for using Google Maps API), as we are replacing just length with time now but other logic to a big degree remains the same.
...
Other than that, another limitation of this project was that roads that ran in both directions, they had their polylines at the same coordinates. This meant that a slight translation had to be done to the polyline depending on if it was of a negative road or positive road to separate bidirectional road's opposite lanes from each other. However, this slight nudge was not uniform everywhere, and sometimes it led to too much of translation whereas other times it led to too little translation of the polyline (despite translation being done using a constant), leading to confusion over which roads are located where in some cases. However, this did not affect the overall goal of this project, but just affected efficiency of the one who uses this, or maybe even slight mistakes if the user of this app is not experienced in using this app that much. This cannot be improved, as Ontario data source officially does not differentiate between polylines of the opposite lands of a bidirectional road segment.  
...
\hangindent=0.5in
\hangafter=1
Gillies, Sean. “Shapely.MultiLineString — Shapely 2.1.2 Documentation.” Readthedocs.io, 2025, \url{shapely.readthedocs.io/en/stable/reference/shapely.MultiLineString.html.} 
...
\hangindent=0.5in
\hangafter=1
---. “St.text - Streamlit Docs.” Streamlit.io, 2024, \url{docs.streamlit.io/develop/api-reference/text/st.text.}
...
\hangindent=0.5in
\hangafter=1
Zwitch, Randy. “Callbacks.” Streamlit.app, 7 July 2023, \url{folium.streamlit.app/callbacks.} 
